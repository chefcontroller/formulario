...
async function loadMes() {
  ...
  if (isSup()) {
    ...
    kpisEl.innerHTML = `
      ...
      <div class="mes-kpi"><div class="mes-kpi-val">${ctotal > 0 ? fmt(vtotal / ctotal) : '—'}</div><div class="mes-kpi-label">Tkt. prom.</div></div>`;
  } else {
  ...
  for (let d = 1; d <= diasEnMes; d++) {
    ...
    for (let nt = 1; nt <= 2; nt++) {
      ...
      tr.innerHTML = `
        ...
        <td style="color:${esFuturo ? 'var(--t3)' : 'var(--t)'}">${esFuturo ? '—' : c ? fmt(venta) : '—'}</td>
...
async function abrirMesTurno(t, c) {
  ...
  if (!c) {
    ...
    document.getElementById('mmtBox').innerHTML = `
      ...
      ${c.cubiertos > 0 ? `<div class="ar"><span class="al">Ticket prom.</span><span class="av">${fmt((c.venta_total||0)/(c.cubiertos||1))}</span></div>` : ''}
      <div class="adiv"></div>
...
function abrirMesTurnoNuevo(fecha, nt, turnoId, turnoObj) {
  ...
  // Turno no existe → modal para elegir número y saldo
